...
# ---------------- STEP 2: IMPROVEMENTS ----------------
    ...
    if st.session_state.photo_stage == "before":
        ...
        st.info("Tap **Browse files**, choose the photo from your Gallery, and wait for the preview. The app saves it immediately after Streamlit receives it.")
        ...
        if uploaded is not None:
            try:
                ...
                st.error(f"This image could not be processed: {type(exc).__name__}: {exc}")
                record = None
    ...
    # AFTER DESCRIPTION / SAVE
        ...
        def collect_current():
            h = st.session_state.get(keys["heading"], "").strip()
            bd = st.session_state.get(keys["before_desc"], "").strip()
            ad = st.session_state.get(keys["after_desc"], "").strip()
