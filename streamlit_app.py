...
if st.session_state.step == 0:
    ...
    # Mobile-friendly upload state. File uploaders are intentionally kept OUTSIDE
    # st.form so Android/iPhone gallery selections are processed immediately.
    ...
    with c1:
        ...
        before = st.file_uploader(
            ...
            type=["jpg", "jpeg", "png"],
            key=f"before_{n}",
            help="For best mobile compatibility, select a JPG, JPEG or PNG photo from your Gallery."
        )
        if before is not None:
            # Immediately copy bytes out of UploadedFile into session state.
            st.session_state.current_uploads[f"before_{n}"] = {
                "data": before.getvalue(),
                "name": before.name
            }
            st.image(before, caption="Before picture selected", use_container_width=True)
        elif f"before_{n}" in st.session_state.current_uploads:
            st.success("Before picture is selected.")
        before_desc = st.text_area(
            "Before Description *",
            key=f"before_desc_{n}",
            height=120,
    ...
    with c2:
        ...
        if after is not None:
            st.session_state.current_uploads[f"after_{n}"] = {
                "data": after.getvalue(),
                "name": after.name
            }
            st.image(after, caption="After picture selected", use_container_width=True)
        elif f"after_{n}" in st.session_state.current_uploads:
            st.success("After picture is selected.")
    ...
    st.markdown("---")
    st.caption("Tip: On mobile, after choosing a photo from Gallery, wait until the photo preview appears before pressing Continue.")

    b1, b2 = st.columns(2)
    with b1:
        cont = st.button("➕ Save & Continue", use_container_width=True)
    with b2:
        end = st.button("✅ Save & Generate PPT", type="primary", use_container_width=True)

    action = "continue" if cont else ("end" if end else None)
    if action:
        errors=[]
        before_saved = st.session_state.current_uploads.get(f"before_{n}")
        after_saved = st.session_state.current_uploads.get(f"after_{n}")
        if not heading.strip(): errors.append("Improvement heading")
        if before_saved is None: errors.append("Before picture")
        if not before_desc.strip(): errors.append("Before description")
        if after_saved is None: errors.append("After picture")
        ...
        if errors:
            ...
            # Clear only the current improvement's upload cache.
            st.session_state.current_uploads.pop(f"before_{n}", None)
            st.session_state.current_uploads.pop(f"after_{n}", None)
            if action == "continue":
                st.rerun()
            else:
