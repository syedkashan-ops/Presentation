...
# Use a non-conflicting key name; SessionState exposes an .items() method.
...
if st.session_state.step == 0:
    ...
    # IMPORTANT MOBILE DESIGN:
    # Only ONE file uploader is displayed at a time. On Android/iPhone,
    # two uploaders side-by-side can make the Gallery picker unreliable.
    # Uploaded bytes are copied into session_state immediately.
    ...
    before_key = f"before_{n}"
    after_key = f"after_{n}"
    before_desc_key = f"before_desc_{n}"
    after_desc_key = f"after_desc_{n}"
    heading_key = f"heading_{n}"
    ...
    if st.session_state.photo_stage == "before":
        st.markdown("### Step 1 of 2 — Before Picture")
        st.info("Tap **Browse files**, choose the photo from your Gallery, and wait for the preview to appear. Only the Before uploader is shown at this stage for better mobile reliability.")

        before = st.file_uploader(
            "Upload Before Picture *",
            type=["jpg", "jpeg", "png", "webp"],
            key=before_key,
            help="JPG/JPEG/PNG/WEBP are recommended for Android Gallery uploads."
        )

        if before is not None:
            try:
                before_bytes = before.getvalue()
                # Validate the image before moving to the next stage.
                test_img = Image.open(BytesIO(before_bytes))
                test_img.verify()
                st.session_state.current_uploads[before_key] = {
                    "data": before_bytes,
                    "name": before.name
                }
                st.image(BytesIO(before_bytes), caption="Before picture selected", use_container_width=True)
                st.success("Before picture uploaded successfully.")
                ...
                st.error("The selected file could not be read as an image. Please choose a JPG, JPEG, PNG or WEBP photo.")

        # If already stored, don't require the Gallery selection again.
        elif before_key in st.session_state.current_uploads:
            data = st.session_state.current_uploads[before_key]["data"]
            st.image(BytesIO(data), caption="Before picture selected", use_container_width=True)
    ...
    # ---------- BEFORE DESCRIPTION ----------
        ...
        st.markdown("### Step 1 of 2 — Before Picture & Description")
        data = st.session_state.current_uploads[before_key]["data"]
        st.image(BytesIO(data), caption="Before picture", use_container_width=True)
        ...
        with c1:
            if st.button("← Replace Before Photo", use_container_width=True):
                ...
                st.session_state.photo_stage = "before"
                # Clear uploader widget state if present.
                st.session_state.pop(before_key, None)
                st.rerun()
        with c2:
            if st.button("Continue to After Photo →", type="primary", use_container_width=True):
                if not before_desc.strip():
    ...
    # ---------- AFTER PHOTO ----------
        ...
        st.markdown("### Step 2 of 2 — After Picture")
        st.info("Now choose the After photo from your Gallery. The Before photo is already saved and will not be affected.")
        ...
        if after is not None:
            try:
                after_bytes = after.getvalue()
                test_img = Image.open(BytesIO(after_bytes))
                test_img.verify()
                st.session_state.current_uploads[after_key] = {
                    "data": after_bytes,
                    "name": after.name
                }
                st.image(BytesIO(after_bytes), caption="After picture selected", use_container_width=True)
                st.success("After picture uploaded successfully.")
            ...
            data = st.session_state.current_uploads[after_key]["data"]
            st.image(BytesIO(data), caption="After picture selected", use_container_width=True)
    ...
    # ---------- AFTER DESCRIPTION / SAVE ----------
        ...
        st.markdown("### Step 2 of 2 — After Picture & Description")
        data = st.session_state.current_uploads[after_key]["data"]
        st.image(BytesIO(data), caption="After picture", use_container_width=True)
        ...
        before_saved = st.session_state.current_uploads.get(before_key)
        after_saved = st.session_state.current_uploads.get(after_key)
        before_desc = st.session_state.get(before_desc_key, "")
        ...
        with b1:
            if st.button("← Replace After Photo", use_container_width=True):
                ...
                st.session_state.pop(after_key, None)
                st.session_state.photo_stage = "after"
                st.rerun()
        with b2:
            if st.button("➕ Save & Continue", use_container_width=True):
                if not before_saved or not before_desc.strip() or not after_saved or not after_desc.strip() or not heading.strip():
                    st.error("Please complete the heading, Before Description and After Description.")
                else:
                    st.session_state.improvements.append({
                        "heading": heading.strip(),
                        "before": before_saved["data"],
                        "before_name": before_saved["name"],
                        "before_desc": before_desc.strip(),
                        "after": after_saved["data"],
                        "after_name": after_saved["name"],
