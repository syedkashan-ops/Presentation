...
# Native mobile photo picker component. This avoids Streamlit's file-uploader
# browser widget and uses the phone's normal image picker directly.
_MOBILE_PICKER = components.declare_component(
    "pso_mobile_photo_picker",
    path=str(Path(__file__).parent / "mobile_photo_picker"),
)

def mobile_photo_picker(label, key, existing=None):
    result = _MOBILE_PICKER(label=label, key=key, existing=existing, default=None, height=120)
    if not result:
        return None
    try:
        ...
        return {"name": result.get("name", "photo"), "type": result.get("type", ""), "bytes": raw}
    except Exception:
        return None
...
def save_uploaded_photo_bytes(raw, filename, key):
    if not raw:
...
def reset_current_draft():
    # Do NOT modify Streamlit widget-owned session_state keys after the widgets
    # have been instantiated.  A new draft_id gives the next draft fresh widget
    # keys and avoids StreamlitWidgetAlreadyInstantiatedError.
...
def current_keys(n):
    did = st.session_state.get("draft_id", 0)
    return f"before_{did}_{n}", f"after_{did}_{n}"

def widget_keys(n):
    did = st.session_state.get("draft_id", 0)
    return f"heading_{did}_{n}", f"before_desc_{did}_{n}", f"after_desc_{did}_{n}"
...
# ---------------- Improvement capture ----------------
    ...
    heading_key, before_desc_key, after_desc_key = widget_keys(n)
    ...
    # Keep a plain application copy separate from widget state.
    st.session_state.draft_heading = heading
    ...
    if st.session_state.photo_stage == "before":
        ...
        before_pick = mobile_photo_picker("Choose Before Photo from Gallery", key=f"picker_{before_key}")
        before = None
        if before_pick is not None:
            before = before_pick
            try:
                save_uploaded_photo_bytes(before["bytes"], before["name"], before_key)
        ...
        st.session_state.draft_before_desc = before_desc
        c1, c2 = st.columns(2)
        with c1:
        ...
        with c2:
            if st.button("Continue to After Photo →", type="primary", use_container_width=True):
                desc = st.session_state.get(before_desc_key, st.session_state.draft_before_desc).strip()
    ...
    # AFTER
        ...
        after_pick = mobile_photo_picker("Choose After Photo from Gallery", key=f"picker_{after_key}")
        after = None
        if after_pick is not None:
            after = after_pick
            try:
                save_uploaded_photo_bytes(after["bytes"], after["name"], after_key)
    ...
    # AFTER DESCRIPTION / SAVE
        ...
        st.session_state.draft_after_desc = after_desc
        ...
        def validate_current():
            # Read the current widget values through their unique keys.
            h = st.session_state.get(heading_key, st.session_state.get("draft_heading", "")).strip()
            bd = st.session_state.get(before_desc_key, st.session_state.get("draft_before_desc", "")).strip()
            ad = st.session_state.get(after_desc_key, st.session_state.get("draft_after_desc", "")).strip()
