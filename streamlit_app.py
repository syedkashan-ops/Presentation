...
# ---------------- STEP 2: IMPROVEMENTS ----------------
    ...
    # Everything for one improvement stays on ONE page.
    # This avoids losing the Before Description when Streamlit reruns after
    # moving between separate photo/description stages.
    ...
    before_file = st.file_uploader(
        "Choose Before Photo from Gallery",
        type=None,
        accept_multiple_files=False,
        key=keys["before_file"],
        help="Select one image. JPG, JPEG, PNG and WEBP are supported. The app compresses the image automatically."
    )

    before_record = st.session_state.current_uploads.get(keys["before_slot"])
    if before_file is not None:
        try:
            before_record = store_upload(before_file, keys["before_slot"])
        except Exception as exc:
            st.error(f"This selected file is not a readable image: {type(exc).__name__}: {exc}")
            before_record = None

    if before_record:
        st.image(before_record["data"], caption="Before picture saved", use_container_width=True)
        st.success("Before picture is saved.")
        st.caption(size_caption(before_record))
    ...
    after_file = st.file_uploader(
        "Choose After Photo from Gallery",
        type=None,
        accept_multiple_files=False,
        key=keys["after_file"],
        help="Select one image. JPG, JPEG, PNG and WEBP are supported. The app compresses the image automatically."
    ...
    after_record = st.session_state.current_uploads.get(keys["after_slot"])
    if after_file is not None:
        try:
            after_record = store_upload(after_file, keys["after_slot"])
        except Exception as exc:
            st.error(f"This selected file is not a readable image: {type(exc).__name__}: {exc}")
            after_record = None

    if after_record:
        st.image(after_record["data"], caption="After picture saved", use_container_width=True)
        st.success("After picture is saved.")
        st.caption(size_caption(after_record))
    ...
    def collect_current():
        # Read the live widget values directly on the same page. The widgets
        # are not removed before Save & Finish, so their values cannot be lost
        # during a stage transition.
        h = (st.session_state.get(keys["heading"], "") or "").strip()
        bd = (st.session_state.get(keys["before_desc"], "") or "").strip()
        ad = (st.session_state.get(keys["after_desc"], "") or "").strip()
    ...
    def validate(values):
        h, bd, ad, b, a = values
        if not h:
            return "Please enter the Improvement Heading."
        if not b:
            return "Please upload the Before Picture."
        if not bd:
            return "Please enter the Before Description."
        if not a:
            return "Please upload the After Picture."
        if not ad:
            return "Please enter the After Description."
        return None
